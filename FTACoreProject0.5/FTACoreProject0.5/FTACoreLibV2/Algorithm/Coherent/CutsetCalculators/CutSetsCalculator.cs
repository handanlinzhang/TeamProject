...
namespace FTACoreLibV2.Algorithm.Coherent.CutsetCalculators
{
    #region 修改记录块
    //修改时间：2013-10-18
    //@修改内容：注释掉aList.Clear()、 bList.Clear()
    //@修改原因：对于存在克隆的子树的故障树，当首先访问源子树（源子树的节点在第二层），然后访问一个包含源引用的节点时，会将源子树的割集清空，影响后面计算

    //@修改时间：2013-10-24
    //@修改原因：对于存在子树引用的故障树，会清空该子树的割集
    //@修改内容：若该中间事件为引用或源，则传其副本进入
    #endregion
