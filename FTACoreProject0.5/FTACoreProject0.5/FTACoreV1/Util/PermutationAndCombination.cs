    ...
    public class PermutationAndCombination<T>
    {
        ...
        private static void GetCombination(ref List<T[]> list, T[] t, int n, int m, int[] b, int M)
        {
            for (int i = n; i >= m; i--)
            {
                ...
                if (m > 1) GetCombination(ref list, t, i - 1, m - 1, b, M);
                else
                {
                    if (list == null) list = new List<T[]>();
                    T[] temp = new T[M];
                    for (int j = 0; j < b.Length; j++) temp[j] = t[b[j]];
                    list.Add(temp);
                }
            }
        }
        ...
        public static List<T[]> GetPermutation(T[] t) { return GetPermutation(t, 0, t.Length - 1); }
